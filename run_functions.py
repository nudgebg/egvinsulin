...
with tqdm(total=len(matched_folders)*6, desc=f"", bar_format='Step {n_fmt}/{total_fmt} [{desc}]:|{bar}', unit="step", leave=False) as progress:
  for folder, study_class in matched_folders:
      ...
      #load data
      progress.set_description_str(f"{study_class.__name__}: (Loading data)")
      ...
      #extract, resample and save
      progress.set_description_str(f"{study_class.__name__}: Extracting boluses")
